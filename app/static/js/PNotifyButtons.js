...
var PNotifyButtons = (function(PNotify) {
  ...
  function _showSticker(_ref) {
    ...
    return (
      sticker && !(_notice && _notice.refs.elem.classList.contains("nonblock"))
    );
  }
  ...
  function _showCloser(_ref2) {
    ...
    return (
      closer && !(_notice && _notice.refs.elem.classList.contains("nonblock"))
    );
  }
  ...
  function _pinDownClass(_ref4) {
    ...
    return _notice
      ? classes.pinDown === null
        ? _notice.get()._icons.pinDown
        : classes.pinDown
      : "";
  }
  ...
  var methods = {
    initModule: function initModule(options) {
      ...
      _notice.on("state", function(_ref6) {
        ...
        var icon = current.hide
          ? _this.get().classes.pinUp
          : _this.get().classes.pinDown;
        if (
          _this.get()._notice.get().icons === "fontawesome5" ||
    ...
    return {
      c: function c() {
        ...
        div.className = div_class_value =
          "ui-pnotify-closer " +
          (!ctx.closerHover || ctx._mouseIsIn
            ? ""
      ...
      p: function p(changed, ctx) {
        ...
        if (
          (changed.closerHover || changed._mouseIsIn) &&
          div_class_value !==
            (div_class_value =
              "ui-pnotify-closer " +
              (!ctx.closerHover || ctx._mouseIsIn
        ...
        if (
          changed.labels &&
          div_title_value !== (div_title_value = ctx.labels.close)
        ) {
    ...
    return {
      c: function c() {
        ...
        span.className = span_class_value =
          "" +
          (ctx._options.hide ? ctx._pinUpClass : ctx._pinDownClass) +
          " svelte-1yjle82";
        addListener(div, "click", click_handler);
        div.className = div_class_value =
          "ui-pnotify-sticker " +
          (!ctx.stickerHover || ctx._mouseIsIn
            ? ""
            : "ui-pnotify-buttons-hidden") +
          " svelte-1yjle82";
        setAttribute(div, "role", "button");
        setAttribute(
          div,
          "aria-pressed",
          (div_aria_pressed_value = ctx._options.hide)
        );
        div.tabIndex = "0";
        div.title = div_title_value = ctx._options.hide
          ? ctx.labels.stick
          : ctx.labels.unstick;
      },
      m: function m(target, anchor) {
      ...
      p: function p(changed, ctx) {
        if (
          (changed._options || changed._pinUpClass || changed._pinDownClass) &&
          span_class_value !==
            (span_class_value =
              "" +
              (ctx._options.hide ? ctx._pinUpClass : ctx._pinDownClass) +
        ...
        if (
          (changed.stickerHover || changed._mouseIsIn) &&
          div_class_value !==
            (div_class_value =
              "ui-pnotify-sticker " +
              (!ctx.stickerHover || ctx._mouseIsIn
        ...
        if (
          (changed._options || changed.labels) &&
          div_title_value !==
            (div_title_value = ctx._options.hide
              ? ctx.labels.stick
              : ctx.labels.unstick)
        ) {
          div.title = div_title_value;
  ...
  function PNotifyButtons(options) {
    ...
    this._recompute(
      { sticker: 1, _notice: 1, closer: 1, classes: 1 },
      this._state
    );
    this._intro = true;
  ...
  PNotifyButtons.prototype._recompute = function _recompute(changed, state) {
    if (changed.sticker || changed._notice) {
      if (
        this._differs(
          state._showSticker,
          (state._showSticker = _showSticker(state))
        )
      )
        changed._showSticker = true;
    }
    ...
    if (changed.closer || changed._notice) {
      if (
        this._differs(
          state._showCloser,
          (state._showCloser = _showCloser(state))
        )
      )
        changed._showCloser = true;
    }
    ...
    if (changed.classes || changed._notice) {
      if (
        this._differs(
          state._pinUpClass,
          (state._pinUpClass = _pinUpClass(state))
        )
      )
        changed._pinUpClass = true;
      if (
        this._differs(
          state._pinDownClass,
          (state._pinDownClass = _pinDownClass(state))
        )
      )
        changed._pinDownClass = true;
      if (
        this._differs(
          state._closerClass,
          (state._closerClass = _closerClass(state))
        )
      )
        changed._closerClass = true;
    }
  };
